package transaction

import (
	"encoding/hex"
	"testing"
)

func TestTransactionEditCandidate_Sign(t *testing.T) {
	data := NewEditCandidateData().
		SetPubKey("Mp4ae1ee73e6136c85b0ca933a9a1347758a334885f10b3238398a67ac2eb153b8").
		MustSetOwnerAddress("Mxe731fcddd37bb6e72286597d22516c8ba3ddffa0").
		MustSetRewardAddress("Mx89e5dc185e6bab772ac8e00cf3fb3f4cb0931c47")

	tx, err := NewBuilder(TestNetChainID).NewTransaction(data)
	if err != nil {
		t.Fatal(err)
	}

	nonce := uint64(1)
	gasPrice := uint8(1)

	transaction := tx.SetNonce(nonce).SetGasPrice(gasPrice).SetGasCoin("MNT")

	privateKey, err := hex.DecodeString("a3fb55450f53dbbf4f2494280188f7f0cd51a7b51ec27ed49ed364d920e326ba")
	if err != nil {
		t.Fatal(err)
	}

	key, err := ToECDSA(privateKey)
	if err != nil {
		t.Fatal(err)
	}

	signedTx, err := transaction.Sign(key)
	if err != nil {
		t.Fatal(err)
	}

	validSignature := ""
	bytes, err := signedTx.Encode()
	if err != nil {
		t.Fatal(err)
	}
	if string(bytes) != validSignature {
		t.Errorf("EncodeTx got %s, want %s", string(bytes), validSignature)
	}
}
